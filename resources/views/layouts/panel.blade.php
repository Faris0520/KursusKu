...
<header class="panel-topbar">
    ...
    <div style="position:relative;" x-data="{ open: false }">
        <button class="panel-user-btn" @click="open = !open" style="border:none;">
            <div style="width:28px;height:28px;border-radius:50%;overflow:hidden;background:#e0e7ff;display:flex;align-items:center;justify-content:center;flex-shrink:0;">
                @if(auth()->user()->avatar)
                    <img src="{{ asset('storage/' . auth()->user()->avatar) }}" style="width:100%;height:100%;object-fit:cover;">
                @else
                    <span style="font-size:0.75rem;font-weight:700;color:#2563eb;">{{ strtoupper(substr(auth()->user()->name, 0, 1)) }}</span>
                @endif
            </div>
            {{ auth()->user()->name }}
    ...
    <aside class="panel-sidebar" :class="{ 'mobile-open': mobileOpen }">
        @auth
            ...
            @elseif(auth()->user()->isAdmin())
                ...
                <a href="{{ route('admin.mentors.index') }}" class="sidebar-nav-link {{ request()->routeIs('admin.mentors.*') ? 'active' : '' }}">
