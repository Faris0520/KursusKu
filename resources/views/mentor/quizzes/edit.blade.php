<x-panel-layout>
    <div class="panel-breadcrumb">
        ...
        <a href="{{ route('mentor.questions.index', [$course, $quiz]) }}">{{ $quiz->title }}</a> / Edit Soal
    </div>

    <h1 class="panel-page-title" style="margin:8px 0 24px;">Edit Soal</h1>

    <div class="panel-card" style="max-width:680px;">
        <div class="panel-card-body">
            <form method="POST" action="{{ route('mentor.questions.update', [$course, $quiz, $question]) }}">
                @csrf @method('PUT')

                <div class="panel-form-group">
                    <label class="panel-form-label" for="question_text">Pertanyaan</label>
                    <textarea class="panel-form-textarea" id="question_text" name="question_text" rows="3" required>{{ old('question_text', $question->question_text) }}</textarea>
                ...
                <div class="panel-form-group">
                    ...
                    <input class="panel-form-input" id="option_{{ $key }}" name="option_{{ $key }}"
                           value="{{ old('option_' . $key, $question->{'option_' . $key}) }}" required>
                </div>
                @endforeach
                ...
                <div class="panel-form-group">
                    ...
                    <select class="panel-form-select" id="correct_answer" name="correct_answer" required>
                        @foreach(['a' => 'A', 'b' => 'B', 'c' => 'C', 'd' => 'D'] as $key => $label)
                        <option value="{{ $key }}" @selected(old('correct_answer', $question->correct_answer) === $key)>{{ $label }}</option>
                        @endforeach
                    </select>
                </div>

                <div style="display:flex;gap:10px;margin-top:8px;">
                    <button type="submit" class="btn-register">Update Soal</button>
